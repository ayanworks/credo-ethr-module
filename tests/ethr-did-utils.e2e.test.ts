...
describe('Test Ethereum Did Utils', () => {
  it('should validate did spec compliant payload', () => {
    const didDoc = EthereumDIDFixtures.VALID_DID_DOCUMENT.didDocument as unknown as DidDocument
  ...
  it('should detect empty verification method', () => {
    const result = validateSpecCompliantPayload(
      new DidDocument({
        id: 'did:ethereum:testnet:0x4A09b8CB511cca4Ca1c5dB0475D0e07bFc96EF49',
        verificationMethod: [],
      })
    )
  ...
  it('should detect invalid verification method', () => {
    const validDid = 'did:ethereum:testnet:0x4A09b8CB511cca4Ca1c5dB0475D0e07bFc96EF49'
    const result = validateSpecCompliantPayload(
      new DidDocument({
        id: validDid,
        verificationMethod: [
